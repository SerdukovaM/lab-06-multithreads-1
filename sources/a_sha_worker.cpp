...
#ifndef TEMPLATE_A_SHA_WORKER_CPP
#define TEMPLATE_A_SHA_WORKER_CPP
...
a_sha_worker::a_sha_worker(size_t n_thr, size_t N,  bool autorun) {
  if(n_thr > std::thread::hardware_concurrency() || n_thr <= 0) {
    throw std::invalid_argument("n_threads specified exceeds hardware concurrency or is less than or equal to zero");
  };
  n_threads = n_thr;
  n_zeros = N;
  cancelled = false;
...
void a_sha_worker::perform_checks() {
  while(!cancelled){
    ...
    BOOST_LOG_TRIVIAL(trace) << " Thread: " << std::this_thread::get_id() << " Data: " << s << " Hash: " << hash;
    bool missed = false;
    ...
    if (!missed) {
      ...
      results.push_back(nlohmann::json{{"timestamp", std::time(nullptr)}, {"hash", hash}, {"data", s}});
    }
  }
}
